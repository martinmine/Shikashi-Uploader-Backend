...
namespace ShikashiAPI.Controllers
{
    ...
    public class UploadController : UserAuthenticatedController
    {
        ...
        public UploadController(IConfiguration config, IUploadService uploadService, IKeyService keyService, IAuthorizationService authService, IS3Service s3Service)
            : base(keyService)
        {
        ...
        public async Task<IActionResult> UploadFile(ICollection<IFormFile> files)
        {
            ...
            var upload = await uploadService.CreateUpload(file.ContentType, ip, fileName, key.User, file.Length);
